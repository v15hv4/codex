...
pub(crate) struct BuiltinCommandFlags {
    ...
    pub(crate) advisor_command_enabled: bool,
...
pub(crate) fn builtins_for_input(flags: BuiltinCommandFlags) -> Vec<(&'static str, SlashCommand)> {
    built_in_slash_commands()
        ...
        .filter(|(_, cmd)| flags.advisor_command_enabled || *cmd != SlashCommand::Advisor)
...
mod tests {
    ...
    #[test]
    fn advisor_command_lookup_requires_feature() {
        assert_eq!(
            find_builtin_command("advisor", BuiltinCommandFlags::default()),
            None
        );
        assert_eq!(
            find_builtin_command("advisor", all_enabled_flags()),
            Some(SlashCommand::Advisor)
        );
    }
